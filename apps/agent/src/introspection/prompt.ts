export const INTROSPECTION_SYSTEM_PROMPT = [
  ...
  '- `user/{userId}/…` — per-user data, where userId is the exact ID from the transcript (e.g. `user/usr-owner/preferences`)',
  '- `project/…` — shared project knowledge',
  '',
  'User messages in the transcript are labelled `USER [usr-xxx]` — always use that exact ID for per-user memory keys.',
  '**NEVER** invent or guess user IDs from names or other context.',
  '',

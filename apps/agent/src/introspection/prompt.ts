export const buildIntrospectionSystemPrompt = (currentUser?: {
  userId: string;
  role: string;
  name?: string;
}): string => {
  ...
  if (currentUser) {
    ...
    lines.push(
      ...
      '## Current User',
      '',
      `The conversation partner is \`${currentUser.userId}\`${namePart}, role: **${currentUser.role}**.`,
    );
  }
  ...
  lines.push(
    ...
    '## Memory Key Rules',
    '',
    'Memory keys MUST follow these namespaces:',
    '- `agent/…` — agent identity and self-knowledge',
    '- `user/{userId}/…` — per-user data, MUST use the actual userId from Current User (e.g. `user/usr-owner/preferences`)',
    '- `project/…` — shared project knowledge',
    '',
    '**NEVER** invent user IDs from names. Always use the exact userId shown above.',
    '',

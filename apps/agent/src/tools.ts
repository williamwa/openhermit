...
const createFileSearchTool = ({
  ...
  execute: async (_toolCallId, args: FileSearchArgs) => {
    ...
    for (const filePath of candidateFiles) {
      const relativePath = workspace.toRelativePath(filePath);
      ...
      for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
        ...
        while (searchFrom <= line.length) {
          const matchIndex = line.indexOf(args.pattern, searchFrom);
          ...
          searchFrom = matchIndex + Math.max(args.pattern.length, 1);
        }
      }
    }

...
import type { SessionRuntime, SessionEventEnvelope } from './runtime.js';
    ...
    switch (method) {
      ...
      case 'session.approve': {
        ...
        if (!('respondToApproval' in runtime)) {
          sendError(ws, id, 'INTERNAL_ERROR', 'Runtime does not support approvals.');
          return;
        }
...
export const attachWebSocketServer = (
  ...
  httpServer.on('upgrade', (request: IncomingMessage, socket, head) => {
    ...
    if (url.pathname !== agentLocalRoutes.ws) {
      socket.destroy();
      return;
    }
  ...
  wss.on('connection', (ws: WebSocket) => {
    ...
    ws.on('message', (data) => {
      ...
      void handleRequest(conn, parsed, runtime);
    });
